...
Deno.serve(async (req) => {
  ...
  const { data: claim } = await supabase
    .from('claim_requests')
    .select('id, listing_id, user_id, status, justification, contact_proof, created_at')
    .eq('id', body.id)
    .maybeSingle();
  if (!claim) return Response.json({ error: 'unknown claim' }, { status: 404 });

  const item = await describeListing(supabase, claim.listing_id);
  ...
  const name = item?.title ?? 'una ficha';
