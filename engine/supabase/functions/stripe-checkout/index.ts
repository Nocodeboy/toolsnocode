...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { price_id, success_url, cancel_url, mode, listing_id } = await req.json();
    ...
    if (!authHeader?.startsWith('Bearer ')) {
      return corsResponse({ error: 'Missing or invalid Authorization header' }, 401, reqOrigin);
    }
    ...
    /**
     * El Boost impulsa una herramienta concreta, y `listing_id` es lo que el
     * webhook lee de los metadatos para activarlo. Dos formas de cobrar sin
     * activar nada, ambas silenciosas para quien paga:
     *
     *  - sin `listing_id`: la suscripción se crea y el webhook no tiene a qué
     *    aplicarla;
     *  - con el `listing_id` de otro: `activateBoost` filtra por `user_id`, así
    ...
    if (!listing_id || typeof listing_id !== 'string') {
      return corsResponse({ error: 'listing_id is required: a boost has to point at a tool' }, 400, reqOrigin);
    }
    ...
    sessionParams.metadata = { listing_id };
    if (mode === 'subscription') {
      sessionParams.subscription_data = { metadata: { listing_id } };
    }
