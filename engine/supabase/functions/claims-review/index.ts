...
import { describeListing, emailMatchesSite } from '../_shared/claims.ts';
 ...
 */
const SITE = Deno.env.get('SITE_URL') ?? 'https://example.com';

const supabase = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);

/** Las dos piezas de reclamaciones comparten cómo se describe una ficha. */
const describeItem = (listingId: string) => describeListing(supabase, listingId);

/** La URL pública de una ficha, para poder abrirla desde el correo. */
const itemUrl = (slug: string | null) => (slug ? `${SITE}/listings/${slug}` : null);
...
Deno.serve(async (req) => {
  ...
  if (req.method === 'GET') {
    ...
    const { data, error } = await supabase
      .from('claim_requests')
      .select('id, listing_id, user_id, status, justification, contact_proof, admin_note, created_at, reviewed_at')
      .eq('status', status)
    ...
    const out = [];
    for (const r of rows) {
      const item = await describeItem(r.listing_id);
  ...
  const item = await describeItem(claim.listing_id);
  if (decision === 'approve') {
    ...
    const { error: ownErr } = await supabase.from('listings').update({ user_id: claim.user_id }).eq('id', claim.listing_id);
  ...
  const mail = await notifyClaimant(claim.user_id, decision, updated.admin_note, item);
...
async function notifyClaimant(
  ...
  const url = itemUrl(item?.slug ?? null);

  const { ok, error } = decision === 'approve'
    ? await sendEmail({
      to,
      subject: `You now own the ${name} listing on Directory`,
      text: [
