...
export function createBoostSync(stripe: Stripe, supabase: SupabaseClient) {
  ...
  async function activateBoost(customerId: string, subscription: Stripe.Subscription) {
    ...
    // Sin `listing_id` en los metadatos (Payment Link, panel de Stripe) solo hay
    ...
    let toolId: string | undefined = subscription.metadata?.listing_id;
    if (!toolId) {
      const { data: owned } = await supabase.from('listings').select('id, name').eq('user_id', userId).limit(2);
      if (owned?.length === 1) {
        toolId = owned[0].id;
        console.info(`No listing_id on subscription; user owns exactly one tool (${owned[0].name}), boosting it`);
      } else {
    ...
    if (error) {
      await recordIncident(customerId, subscription.id, 'boost_update_failed', { listing_id: toolId, error: error.message });
      return false;
    }
    if (!count) {
      await recordIncident(customerId, subscription.id, 'tool_not_owned_by_user', { listing_id: toolId, user_id: userId });
      return false;
    }
  ...
  async function syncCustomerFromStripe(customerId: string) {
    ...
    if (['active', 'trialing'].includes(subscription.status)) {
      ...
      await deactivateBoostForCustomer(customerId, subscription.metadata?.listing_id);
